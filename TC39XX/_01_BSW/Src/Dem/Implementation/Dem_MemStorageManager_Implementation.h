...
/* ****************************************************************************
 * Dem_MemStorageManager_UpdateVCCEventEntryOnTriggerFailed
 *****************************************************************************/
/*!
...
# if (DEM_CFG_SUPPORT_CONFIRMED_VISIBLE_FREEZE_FRAME == STD_ON)
        if ((DEM_OPERATIONCYCLE_TEST_DCY_QUALIFIED(Dem_OperationCycle_GetObdCycleStates()) == TRUE)
          || (Dem_Cfg_EventTripTarget(DTCContext->EventId) == 0u))                                                                               
        {
  ...
  )
{
  ...
  */
  ...
  if (Dem_MemoryEntry_GetStdEnvData(lMemoryEntryId, DEM_MEMORYENTRY_STDENVDATA_STATUS) == 0x00U)
  {
    ...
    Dem_MemoryEntry_SetStdEnvData(lMemoryEntryId, DEM_MEMORYENTRY_STDENVDATA_FIRSTODOMETER + 1u, 
                                  Dem_MemoryEntry_GetStdEnvData(lMemoryEntryId, 
  ...
  )
{
    ...
    /* Set SI.30 bit 7 */
  {
    ...
    lBlockModification |= Dem_MemStorageManager_UpdateVCCEventEntryOnTriggerFailed(MemoryIndex, lWirIndicatorTriggered);
  }
# if (DEM_CFG_SUPPORT_USER_ERECS == STD_ON)
