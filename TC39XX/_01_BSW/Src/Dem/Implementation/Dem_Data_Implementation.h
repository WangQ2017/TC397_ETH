  ...
  )
{
  ...
  return ((Dem_Mem_GetPid41DisabledMask() & ((uint16)1 << lReadinessGroup)) != 0u);
}
#endif
...
# endif 
    ...
    {
      uint8 lReadinessGroup;
      for (lReadinessGroup = 0x00U; lReadinessGroup < DEM_CFG_READINESS_GROUP_COUNT; lReadinessGroup++)
      {
  ...
  )
{
  ...
  { /* Event not stored, try to find an empty slot */
    ...
    if (lPermanentIndex != Dem_MemState_MemoryGetMaxSize(lMemoryInfo))                                                           /* SBSW_DEM_CALL_MEMORYINFO_POINTER */
    {
      ...
      if (Dem_Cfg_IsObdIISupportedInVariant() == TRUE)
      {
        /* 0-trip DTCs and DTCs with qualified WIR is immediately visible */
        if ( (0U == Dem_Cfg_EventTripTarget(EventId))
