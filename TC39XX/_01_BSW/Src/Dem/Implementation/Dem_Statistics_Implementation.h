  ...
  )
{
  ...
  if (Dem_MemoryEntry_GetWarmUpCyclesSinceLastFailed(CopyDidInfoPtr->MemoryEntryId) == DEM_OPERATIONCYCLE_CYCLECOUNT_INVALID)
  {
    lWarmUpCyclesSinceLastFailed = 0x00U;
  }
  else
  if (Dem_MemoryEntry_GetWarmUpCyclesSinceLastFailed(CopyDidInfoPtr->MemoryEntryId) == DEM_OPERATIONCYCLE_CYCLECOUNT_LATCHED)
  {
  ...
  else
  {
    lWarmUpCyclesSinceLastFailed = (uint8)Dem_OperationCycle_CycleCountDistance(
                                            Dem_MemoryEntry_GetWarmUpCyclesSinceLastFailed(CopyDidInfoPtr->MemoryEntryId),
