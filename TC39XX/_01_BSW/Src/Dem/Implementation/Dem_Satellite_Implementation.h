...
#define DEM_SATELLITE_IMPLEMENTATION_FILENAME "Dem_Satellite_Implementation.h"
  ...
  )
{
  if (Dem_SatelliteSat_IsLRTimeDebouncingEnabled() == TRUE)
  {
    ...
    if (Dem_Error_isDevRunTimeChecksEnabled() == TRUE)                                                                           
    {
      ...
      {
        Dem_Error_RunTimeCheckFailed(DEM_SATELLITE_IMPLEMENTATION_FILENAME, __LINE__);                                           /* SBSW_DEM_POINTER_RUNTIME_CHECK */
      }
      else
      {
