...
#define DEM_FILTERDATA_IMPLEMENTATION_FILENAME "Dem_FilterData_Implementation.h"
...
# if (DEM_DEV_RUNTIME_CHECKS == STD_ON)
  ...
  {
    Dem_Error_RunTimeCheckFailed(DEM_FILTERDATA_IMPLEMENTATION_FILENAME, __LINE__);                                                                            /* SBSW_DEM_POINTER_RUNTIME_CHECK */
  }
  else
# endif
  {
  ...
  )
{
  ...
  case DEM_CFG_MEMORYID_PRIMARY:
    ...
    { /* reporting shall be in chronological order */
      lFilterData.Iterator.ChronoIter = DEM_FILTERDATA_FILTER_INIT;
 ...
 */
DEM_LOCAL FUNC(Std_ReturnType, DEM_CODE)
Dem_FilterData_FilterForEventMatch(                                                                                              /* PRQA S 2889 */ /* MD_DEM_15.5 */
  CONST(Dem_FilterData_HandleType, AUTOMATIC)  FilterId,
  CONST(Dem_EventIdType, AUTOMATIC)  EventId
  )
{
...
# endif
  ...
  if ( (Dem_FilterData_TestUseStatusMask(FilterId) == TRUE)
    && ((Dem_FilterData_GetStatusMask(FilterId) & lEventStatus) == 0u))
  {
  ...
  if ( (Dem_FilterData_TestUseSeverity(FilterId) == TRUE)
    && ((Dem_FilterData_GetSeverityMask(FilterId) & Dem_Cfg_EventSeverity(EventId)) == 0u))
...
# if (DEM_FEATURE_NEED_REPORT_CHRONOLOGICALLY == STD_ON)
      ...
      if (Dem_FilterData_TestEventReported(FilterId, Dem_Cfg_GetMasterEvent(lEventId)) == FALSE)
# endif
      {
    ...
    { /* write back the next event to process */
      lFilterData.Iterator.EventIndex = (Dem_EventIdType)(lEventId + 1u);
    }
...
# if ((DEM_CFG_SUPPORT_TYPE2_COMBINATION == STD_ON))
      ...
      if (Dem_FilterData_TestEventReported(FilterId, Dem_Cfg_GetMasterEvent(lEventId)) == FALSE)
      {
        if(Dem_FilterData_FilterForEventMatch(FilterId, Dem_Cfg_GetMasterEvent(lEventId)) == E_OK)
        {
          /* identified an Event which matches the filter criteria, mark event as reported */
          Dem_FilterData_SetEventReported(FilterId, Dem_Cfg_GetMasterEvent(lEventId));
          break;
        }
        else
  ...
  )
{
  ...
  {
    if ( (Dem_Cfg_IsObdIISupportedInVariant() == TRUE)
      && (Dem_FilterData_TestUseFormatOBD(FilterId) == TRUE) )
    {
...
# if (DEM_CFG_REPORT_SUPPRESSED_DTCS_IN_19_0A == STD_ON)
    if ( (Dem_FilterData_TestUseStatusMask(FilterId) == FALSE)
      && (Dem_FilterData_GetMemoryId(FilterId) == DEM_CFG_MEMORYID_PRIMARY)                                                      
  ...
  )
{
  ...
  if (lEventId != DEM_EVENT_INVALID)
  {
    ...
    {
      if (lFilterData.NumberOfDTCsTested < Dem_Cfg_GlobalJ1939ReadDtcRuntimeLimit())
      {
# if (DEM_CFG_SUPPORT_TYPE1_COMBINATION == STD_ON)
...
# endif
        {
          ++lFilterData.NumberOfDTCsTested;
        }
      ...
      else
      {
        ...
        lFilterData.Iterator.EventIndex = lEventId;
