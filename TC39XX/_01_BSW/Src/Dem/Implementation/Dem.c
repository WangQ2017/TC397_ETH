...
#if ( (DEM_SW_MAJOR_VERSION != 19) \
   || (DEM_SW_MINOR_VERSION !=  7) \
...
#if ( (DEM_CFG_SW_MAJOR_VERSION != 19) \
   || (DEM_CFG_SW_MINOR_VERSION !=  7) )
# error "Version numbers of Dem.c and Dem_Cfg.h are inconsistent!"
#endif

#if (DEM_CFG_GEN_MAJOR_VERSION != 19)
...
/* module specific MISRA deviations:
  ...
  MD_DEM_14.2_LoopControl
   ...
   MD_DEM_9.1_fp:
     ...
     Reason:      Preceding return value check blocks usage of uninitialized object. Classified as False Positive.
     Risk:        Dem may report incorrect Fdc value.
     Prevention:  None.
  ...
  MD_DEM_2897:
   ...
   MD_DEM_6060_COPYDIDINFO:
     Description: Metrics
                  Number of function parameters defined by HIS shall be in range 0..5
     Reason:      Copy DID Info struct is used in different contexts (Swc, Dcm) of requesting different (single vs all records) 
                  event record data. This multi-purpose usage requires passing many context information. Nonetheless, the DID 
                  collection works same for all records. 
                  Number of parameters can be reduced by splitting the implementation to context specific variants.
                  Code readability will benefit too. Code dublication might be introduced. 
                  Restructuring requires larger design and implementation reworks.
     Risk:        Stack usage and runtime too high for target uC. Understandability and testability might become too complex.
     Prevention:  Design and code review. Fix with Jira task DEM-4275.
