...
DEM_LOCAL VAR(uint32, DEM_VAR_NOINIT)            Dem_Timer_TimeSeriesNormalRate;
# if (DEM_FEATURE_NEED_TIME_SERIES_FAST == STD_ON)
/*! Fast rate time-series snapshot record update poll timer */
DEM_LOCAL VAR(uint32, DEM_VAR_NOINIT)            Dem_Timer_TimeSeriesFastRate;
# endif
#endif
...
# if (DEM_FEATURE_NEED_TIME_SERIES == STD_ON)
  ...
  if (Dem_Timer_TimeSeriesNormalRate == 0u)
  {
...
#  if (DEM_FEATURE_NEED_TIME_SERIES_FAST == STD_ON)
  ...
  if (Dem_Timer_TimeSeriesFastRate == 0u)
  {
...
/* <<<< -------------------------------- Leave Critical Section: DiagMonitor */
      ...
      while (lPid01UpdateFlag != 0u)
      {
        if ((Dem_Data_TestReadinessDynamicallyCalculated(lGroupIter, DEM_DATA_PIDCONTEXT_PID01) == TRUE)
            && (Dem_Esm_TestPid01ReadinessState(lPid01UpdateFlag, lGroupIter) == TRUE))
        {
          ...
          /* Reset flag of this readiness group  */
          lPid01UpdateFlag = Dem_Esm_ResetPid01ReadinessState(lPid01UpdateFlag, lGroupIter);
        }
