...
Dem_MemoryRestoration_RecoverRootCauseEventIdFromDID(
  ...
  CONSTP2VAR(Dem_DataReportIF_CopyDidInfoType, AUTOMATIC, AUTOMATIC) CopyDidInfoPtr
  );
#endif
...
# if (DEM_CFG_SUPPORT_SRECS == STD_ON)
       ...
       Dem_SnapshotEntry_IteratorNext(&lSnapshotEntryIter))                                                                      /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    if (Dem_SnapshotEntry_IteratorIsSnapshotRecordStored(&lSnapshotEntryIter) == TRUE)                                            /* SBSW_DEM_POINTER_FORWARD_STACK */
    {
      ...
      lCopyDidInfo.SourceBuffer = Dem_MemoryEntry_GetSnapshotRecordDataPtr(MemoryEntryId, lSnapshotEntryIndex);                  /* SBSW_DEM_POINTER_WRITE_MEMORYENTRY */
           ...
           Dem_Cfg_EventDidIterNext(&lDidIter))                                                                                  /* SBSW_DEM_CALL_ITERATOR_POINTER */
      {
        ...
        lRootCauseEventId = Dem_MemoryRestoration_RecoverRootCauseEventIdFromDID(lDidIndex, &lCopyDidInfo);                      /* SBSW_DEM_POINTER_WRITE_MEMORYENTRY */
       ...
       Dem_Cfg_DidDataIterNext(&lDataIter))                                                                                      /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if (Dem_Cfg_DataIsStoredInNV(lDataIndex) == TRUE)
    {
      ...
      case DEM_CFG_DATA_FROM_ROOTCAUSE_EVENTID:
      {
        ...
        lHighByte = CopyDidInfoPtr->SourceBuffer[CopyDidInfoPtr->ReadIndex];
        lLowByte = CopyDidInfoPtr->SourceBuffer[CopyDidInfoPtr->ReadIndex + 1u];
       ...
       Dem_ExtendedEntry_IterNext(&lExtendedEntryIter))                                                                          /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    if (Dem_Cfg_ERecType(lCfgERecIndex) == DEM_CFG_EREC_TYPE_USER)
    {
      if (Dem_ExtendedEntry_IteratorIsExtendedRecordStored(MemoryEntryId, lExtendedEntryIndex) == TRUE)                           /* SBSW_DEM_POINTER_FORWARD_STACK */
      {
        ...
        lCopyDidInfo.SourceBuffer = Dem_MemoryEntry_GetExtendedRecordDataPtr(MemoryEntryId, lExtendedEntryIndex);                /* SBSW_DEM_POINTER_WRITE_MEMORYENTRY */
        lCopyDidInfo.ReadIndex = 0;

        lDidIndex = Dem_Cfg_ERecDid(lCfgERecIndex);
        {
          lRootCauseEventId = Dem_MemoryRestoration_RecoverRootCauseEventIdFromDID(lDidIndex, &lCopyDidInfo);                    /* SBSW_DEM_POINTER_LOCAL_COPYDIDINFO */
