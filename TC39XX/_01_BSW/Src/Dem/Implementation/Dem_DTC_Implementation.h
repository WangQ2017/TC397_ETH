...
#define DEM_DTC_IMPLEMENTATION_FILENAME "Dem_DTC_Implementation.h"
...
# if (DEM_DEV_RUNTIME_CHECKS == STD_ON)
  ...
  {
    Dem_Error_RunTimeCheckFailed(DEM_DTC_IMPLEMENTATION_FILENAME, __LINE__);                                                     /* SBSW_DEM_POINTER_RUNTIME_CHECK */
  }
  else
# endif
  {
...
)
{
    ...
    Dem_Cfg_CombinedGroupIterNext(&lCombinedIter))                                                                               /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
      ...
      && (Dem_EventInternalStatus_TestEventDisconnected(Dem_Event_GetInternalStatus(lSubEventId)) == FALSE))                     
    {
      ...
      if ((lReturnValue == E_OK) && (lMaxFdc < lFdc))                                                                            /* PRQA S 2962 */ /* MD_DEM_9.1_fp */
      {
        lMaxFdc = lFdc;
      }
    }
...
#if ((DEM_FEATURE_NEED_OEM_EXTENSIONS_VCC == STD_ON) || (DEM_CFG_SUPPORT_TRIGGER_PASSED == STD_ON))
/* ****************************************************************************
...
#endif /*DEM_ORGANI_OEM_VCC_SUPPORT_COMMENT == STD_ON*/
...
#if ((DEM_CFG_SUPPORT_TRIGGER_PASSED == STD_ON) && (DEM_FEATURE_NEED_OEM_EXTENSIONS_VCC == STD_OFF))
...
#  endif
      )
    {
      ...
      /* >>>> -------------------------------- Enter Critical Section: DiagMonitor */
      Dem_EnterCritical_DiagMonitor();
      lNewEventInternalStatus = Dem_Event_GetInternalStatus(EventId);
