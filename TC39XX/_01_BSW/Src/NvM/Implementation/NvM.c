...
# endif /* ECO_IGNORE_LINE */
{
    ...
    if (NvM_IsMaskSet(
        ...
        NVM_BLOCK_WRITE_BLOCK_ONCE_ON) == FALSE)
    {
...
# endif /* ECO_IGNORE_LINE */
  {
    const NvM_BitFieldType blockDescriptorFlags = NvM_BlockDescriptorTable_at[NVM_BLOCK_FROM_DCM_ID(BlockId)].Flags;

    if (NvM_IsMaskSet(blockDescriptorFlags, NVM_BLOCK_USE_SET_RAM_BLOCK_STATUS_ON) == TRUE)
    {
...
# if (NVM_CALC_RAM_CRC_USED == STD_ON)
        if (BlockChanged && (NvM_IsMaskSet(blockDescriptorFlags, NVM_CALC_RAM_BLOCK_CRC_ON) == TRUE)) /* PRQA S 3415 */ /* MD_NvM_13.5 */
        {
...
FUNC(void, NVM_PUBLIC_CODE) NvM_MainFunction(void)
{
...
#endif /* ECO_IGNORE_LINE */
    {
        ...
        if(!NvM_CrcJob_isBusy(&NvM_AsyncCrcJob_t))
        {
             ...
             *       We may omit a special check here                                    */
            NvM_CrcJob_Create(&NvM_AsyncCrcJob_t, newCrcBlockId, descr_pt->RamBlockDataAddr_t, descr_pt->NvBlockLength_u16); /* SBSW_NvM_FuncCall_PtrParam_CrcAsyncJob */
