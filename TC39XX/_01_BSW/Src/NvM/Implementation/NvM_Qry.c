...
/**********************************************************************************************************************
 * NvM_QryDoRamCrcCheck
 *********************************************************************************************************************/
/*! \brief Checks if the RAM block CRC must be checked.
 *  \details -
 *  \return TRUE if the calculate RAM block CRC feature is enabled, FALSE otherwise.
 *  \context TASK
 *  \reentrant FALSE
 *  \synchronous TRUE
 *  \pre -
 */
NVM_LOCAL FUNC(boolean, NVM_PRIVATE_CODE) NvM_QryDoRamCrcCheck(void);
 ...
 *  \reentrant FALSE
 *  \synchronous TRUE
 *  \config SetRamBlockStatus API enabled
 *  \pre -
 */
...
NVM_LOCAL FUNC(boolean, NVM_PRIVATE_CODE) NvM_QryBlockRelevantForWriteAll(void)
{
  ...
  boolean isSelectedForWriteAll = NvM_IsMaskSet(
      NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_SELECT_BLOCK_FOR_WRITEALL_ON);
  ...
  {
#if (NVM_SET_RAM_BLOCK_STATUS_API == STD_ON)
    boolean useSetRamBlockStatus = NvM_IsMaskSet(
        NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_BLOCK_USE_SET_RAM_BLOCK_STATUS_ON);

    if (useSetRamBlockStatus)
    {
      /* Block using NvMBlockUseSetRamBlockStatus feature -> write only if changed data was indicated via API */
      retVal_isRelevantForWriteAll = NvM_IsMaskSet(
...
NVM_LOCAL FUNC(boolean, NVM_PRIVATE_CODE) NvM_QryDoRamCrcCheck(void)
{
    /* Note: A busy CRC job implies the block has CRC enabled. */
    return (boolean)(NvM_IsMaskSet(NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_CALC_RAM_BLOCK_CRC_ON));
}
#endif /* NVM_SET_RAM_BLOCK_STATUS_API == STD_ON */
...
{
  return (boolean)(
      (NvM_IsMaskSet(NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_SELECT_BLOCK_FOR_READALL_ON) == FALSE)
...
NVM_LOCAL FUNC(boolean, NVM_PRIVATE_CODE) NvM_QryWriteBlockOnce(void)
{
  ...
  if (NvM_IsMaskSet(NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_BLOCK_WRITE_BLOCK_ONCE_ON) == TRUE)
  {
#if(NVM_DYNAMIC_CONFIGURATION == STD_ON) /* ECO_IGNORE_LINE */
...
NVM_LOCAL FUNC(boolean, NVM_PRIVATE_CODE) NvM_QryRamValid(void)
{
  return (boolean)((NvM_CurrentBlockInfo_t.Descriptor_pt->RamBlockDataAddr_t != NULL_PTR) &&
      ((NvM_CurrentBlockInfo_t.Mngmt_pt->NvRamAttributes_u8 & NVM_STATE_VALID_SET) != 0u)
      && ((NvM_CurrentBlockInfo_t.Mngmt_pt->NvRamAttributes_u8 & NVM_LOCK_STAT_SET) == 0u));
}
#endif /* (NVM_SET_RAM_BLOCK_STATUS_API == STD_ON) */
...
NVM_LOCAL FUNC(boolean, NVM_PRIVATE_CODE) NvM_QryHasRom(void)
{
  return (
      ...
      || (NvM_QryIsInitCallbackConfigured(NvM_CurrentBlockInfo_t.Descriptor_pt) /* PRQA S 3415 */ /* MD_NvM_13.5 */
          && NvM_IsMaskSet(NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_INVOKE_CALLBACKS_FOR_READALL_ON)) /* PRQA S 3415 */ /* MD_NvM_13.5 */
...
NVM_LOCAL FUNC(boolean, NVM_PRIVATE_CODE) NvM_QryExtRuntime(void)
{
  ...
  return (boolean)(
      ...
      (NvM_IsMaskSet(NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_RESISTANT_TO_CHANGED_SW_ON) == FALSE)); /* PRQA S 3415 */ /* MD_NvM_13.5 */
#else
