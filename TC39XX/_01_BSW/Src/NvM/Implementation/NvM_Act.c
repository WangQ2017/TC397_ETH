...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActFinishReadBlock(void)
{
    ...
    if(NvM_IsMaskSet(descr_pt->Flags, NVM_BLOCK_WRITE_BLOCK_ONCE_ON) == TRUE)
    {
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActRestoreRomDefaults(void)
{
  ...
  else
  {
    ...
    {
      const boolean allowReadAllInvocation = NvM_IsMaskSet(
          NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_INVOKE_CALLBACKS_FOR_READALL_ON);
...
NVM_LOCAL FUNC(void, NVM_PRIVATE_CODE) NvM_ActSetInitialAttr(void)
{
    ...
    if (NvM_IsMaskSet(NvM_CurrentBlockInfo_t.Descriptor_pt->Flags, NVM_BLOCK_WRITE_PROT_ON) == TRUE)
    {
...
void NvM_BlockNotification(NvM_BlockIdType BlockId, NvM_ServiceIdType ServiceId, NvM_RequestResultType JobResult)
{
  ...
  {
    const boolean allowReadAllInvocation = NvM_IsMaskSet(blockDescriptorPtr->Flags, NVM_INVOKE_CALLBACKS_FOR_READALL_ON);
    const boolean allowWriteAllInvocation = NvM_IsMaskSet(blockDescriptorPtr->Flags, NVM_INVOKE_CALLBACKS_FOR_WRITEALL_ON);
    ...
    if(blockDescriptorPtr->NotifyBswM == STD_ON)
    {
