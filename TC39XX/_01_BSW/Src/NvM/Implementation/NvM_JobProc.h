...
/*! Extern definition of variables that area used in the NvM submodules - they are defined in NvM.c. */
 ...
 *********************************************************************************************************************/
/*!
 * \brief      Tests if a mask is set in a bit field.
 * \details    -
 * \param[in]  BitField     Bit field to test if mask is set.
 * \param[in]  MaskToCheck  Mask to be tested.
 * \return     TRUE if mask is set; FALSE if mask is not set.
 * \pre        -
 * \context    TASK
 * \reentrant  TRUE
 * \synchronous TRUE
 */
FUNC(boolean, NVM_PRIVATE_CODE) NvM_IsMaskSet(const NvM_BitFieldType BitField, const NvM_BitFieldType MaskToCheck);
