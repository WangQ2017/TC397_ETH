 ...
 *        SW-C Type:  Cdd_PPS
 *  Generation Time:  2026-06-07 17:20:22
 *
    ...
    switch(g_ppsCurStatus)
    {
      case PPS_UNSYNC:
      {
        ...
        if(StbM_BusGetCurrentTime(0, &currGlobalTime, &currVlt, &userData) == E_OK) /* SBSW_ETHTSYN_REFERENCE_OF_LOCAL_VARIABLE */
        {
            ...
            ntGap = (uint32)(PPS_NANO_PER_SECOND - (glbTime % PPS_NANO_PER_SECOND));
            s_nexTime.ntNSec = (uint32)((virLocalTime + ntGap) % PPS_NANO_PER_SECOND);
            s_nexTime.ntSec = (uint32)((virLocalTime + ntGap) / PPS_NANO_PER_SECOND);
            s_nexTime.nsPerSec = (uint32)(PPS_NANO_PER_SECOND);
            s_nexTime.ntGlbTimeSec = (glbTime / PPS_NANO_PER_SECOND) + 1;
      ...
      case PPS_LOW:
      {
        ...
        {
          CDD_LOG_ERROR("pps set fail in pps low!");
          GETH_MAC_PPS_CONTROL.B.PPSCTRL_PPSCMD = 5;
          g_ppsCurStatus = PPS_UNSYNC;
          return;
        }
